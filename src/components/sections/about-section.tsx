...
export function AboutSection() {
  return (
    <SceneSection id="about" variant="alpine" overlay="both">
      <div className="flex-1 flex items-center px-6 md:px-12 py-24">
        <div className="mx-auto w-full max-w-7xl grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          >
            <p className="font-mono text-[11px] uppercase tracking-[0.3em] text-zinc-300/80 mb-6 drop-shadow-md">
              <span className="text-accent">◆</span>&nbsp;&nbsp;Chapter 01 — Origin
            </p>
            <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-[10rem] font-black tracking-tight leading-[0.85] text-white drop-shadow-2xl mb-8">
              Alfonso
              <br />
              <span className="text-white/90">Mayoral</span>
            </h1>
          ...
          >
            <div className="relative">
              <div className="absolute -inset-2 rounded-full bg-white/40 blur-3xl" />
              <Image
                src="/profile.png"
                alt={PROFILE.name}
                width={320}
                height={320}
                priority
                className="relative h-64 w-64 md:h-80 md:w-80 rounded-full object-cover border-2 border-white/60 shadow-2xl"
              />
            </div>
          </motion.div>
        </div>
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <p className="font-mono text-[10px] uppercase tracking-[0.25em] text-white/60 mb-1.5 drop-shadow">
        {label}
      </p>
